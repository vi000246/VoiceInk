...
enum TierPrompts {
    ...
    /// aboutMe cue 的 Tier 1 system。格式段落與 `tier1System` **完全相同**(OPENER + 恰 3 bullets),
    /// 所以 `TierParsers.parseTier1` 不必改動;換掉的只有「這是什麼題目、可以用什麼素材」。
    ///
    ...
    static func tier1SystemAboutMe(persona: String,
        ...
        下方「我的筆記」與「我的自介」是**唯一事實來源**——只能使用它們,筆記沒記載的直接說
        「筆記沒記」,絕不編造(編造我的經歷會被當場拆穿,寧可空手)。
        目標:讓我能在 2 秒內開口。輸出**極簡**,只給:
        1. 一句「開口稿」——我可以直接照著說出口的第一句話,用來爭取思考時間,不要長。
        2. 恰好 3 個**記憶錨點**,每點一行、以「- 」開頭、每點不超過 20 字。
           錨點給:專案名／我的角色／具體貢獻／量化成果——看到這些我自己會想起細節,
           不要寫成論述句或替我把話講完。
        嚴格格式(不要任何其他文字、不要 markdown):
        OPENER: <一句可直接說出口的話>
        - <記憶錨點一>
