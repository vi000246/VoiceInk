...
| ask-ai | [ask-ai.spec.md](./ask-ai.spec.md) | Supporting Domain | Semantic QA over all transcription history: chunk → cloud embeddings (BYOK, Gemini default) → local vector index (4th SwiftData store) → brute-force top-k → cited answers. Implemented (build 231); enhancements (answer model / single-recording / Ask AI templates) shipped；Obsidian 筆記一級語料庫（雙 chip scope／筆記設定收編／引用回 Obsidian）spec'd. | — |
...
|------|--------|-------------|-----------------|
| 2026-07-14 | meeting-copilot | [m9-aboutme-replay-lifecycle.srs.md](../srs/meeting-copilot-m9-aboutme-replay-lifecycle.srs.md) | **M9 spec'd：aboutMe 嚴格接地＋覆盤生命週期＋深答風格** — aboutMe 零筆記片段 → 不呼叫 LLM 直接「筆記沒記載」（結構性防幻覺）、1~3 條錨點不硬湊、不進 Tier 2；覆盤標題 live lookup 錄音名（改名自動反映）、刪錄音全刪關聯 session（live＋replay）、live/replay 不同時並存、`MeetingReplayQueue` 背景佇列＋雙頁「覆盤中」tag；深答風格三選一（條列式預設／簡潔／詳細，JSON 契約不變）。FR-65~73 / AC-48~58。 |
