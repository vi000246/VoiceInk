...
|------|--------|-------------|-----------------|
| 2026-07-13 | meeting-copilot | [m2-cue-detection.srs.md](../srs/meeting-copilot-m2-cue-detection.srs.md) | **M1–M5 全數 implemented（build 252）** — 五個里程碑的引擎與 UI 皆建置且測試通過：M1 音訊分流骨幹（248）、M2 cue 偵測 + meeting.store（249）、M3 三層回應 + 自持 SSE + 接地（250）、M4 隱蔽 overlay + 熱鍵（251）、M5 管理頁 + 設定（252）。**剩一個執行期整合 task**（把元件接成 live pipeline，需 make deploy + 真會議驗證）+ 人工 gate（tapFirst probe、AC-15 螢幕分享三情境）。見各 `docs/reports/meeting-copilot-m*-report.md`。 |
