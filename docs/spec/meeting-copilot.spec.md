...
|------|------------|-------------|---------|
| 2026-07-14 | N/A（使用者需求） | `docs/srs/meeting-copilot-m9-aboutme-replay-lifecycle.srs.md` | **M9 spec'd：aboutMe 嚴格接地＋覆盤生命週期＋深答風格（未實作）** — (A) aboutMe 嚴禁幻覺（使用者實測：筆記索引未建時 aboutMe 亂答）：接地零筆記片段 → **不呼叫 LLM**（`runTier1` 內結構性守門，非 prompt 祈禱），固定回「筆記沒記載」、`aboutMeBrief` 非空時為唯一條列；有片段 → 1~3 條記憶錨點**不硬湊**（原「恰 3 bullets」正是逼模型編造的來源之一）；aboutMe **不進 Tier 2**（`runAutoDeep`/`requestDeep` 雙守門＝引擎層涵蓋所有 UI 入口；`tier2SystemAboutMe`/`tier2UserAboutMe` 變體移除）。(B) 覆盤生命週期：session 標題顯示時以 `importFingerprint` live lookup 錄音顯示名（`recorderTitle` 優先；改名自動反映、**不存副本**；查不到退回日期命名）；`.transcriptionDeleted` → sweep 刪該 fingerprint 的 **live＋replay session 全刪**（使用者定案；空 fingerprint 永不動；鏡射 `reconcileOrphans` 模式）；live 與 replay **不同時並存**（存在 live → 隱藏「產生／重新產生」；刪掉 live 後按鈕回來——**狀態判斷非永久規則**；無 live 時多場 replay A/B 並存保留）；覆盤從詳情 sheet 的 @State service 升級為 `MeetingReplayQueue` 背景 FIFO 佇列（關 sheet 不中斷、同錄音防重複、完成不自動跳轉、失敗通知＋既有整場刪語意不變），錄音管理＋覆盤頁雙邊「覆盤中」badge、完成即除。(C) 深答風格（對話中讀不完大段分析）：新鍵 `meetingCopilotDeepStyleV1` 三選一——條列式（**預設**，關鍵詞開頭 ≤4 條＋followUps ≤2，3 秒掃視目標）／簡潔（2~3 句結論）／詳細（現行 prompt **逐字不變**＝回歸鎖）；只抽換 `tier2System` 格式指示段，JSON 契約與 parser／overlay 零改動。FR-65~73 / AC-48~58。 |
| 2026-07-14 | N/A（ask-ai 跨模組） | `docs/srs/ask-ai-obsidian-notes-rag.srs.md` | **筆記 RAG 管線設定所有權移交 ask-ai（spec'd，未實作）** — Ask AI 把 obsidian 筆記升級為一級查詢語料庫，共用管線設定（vault override＋include/exclude 資料夾）收編到新 `ObsidianRAGConfigStore`（UserDefaults 鍵沿用、零遷移）。本模組保留消費開關（`useNotesRAG`／`notesInTechnicalRAG`／`aboutMeBrief`）；設定頁筆記 section 瘦身＝開關＋自介＋「筆記索引設定（Ask AI）→」導航連結（`AppNavigator.navigate(to: .askAI)`）；`scheduleNotesReindex` 改讀新 store 與 effective vault（override ?? 錄音 vault）；`notesIndexStateURL` 搬到 `ObsidianNoteIndexService`（sidecar 路徑不變）。aboutMe／技術 cue 的 `gather(sources:)` 檢索白名單機制不變。 |
...
| **空接地守門**（M9） | aboutMe 的結構性防幻覺：接地結果零筆記片段 → **不呼叫 LLM**，直接寫固定「筆記沒記載」回應（自介非空時為唯一條列）。prompt 只能降低機率，不呼叫模型才是保證。 |
| **覆盤刪除傳播**（M9） | 錄音在錄音管理被刪 → 該 `importFingerprint` 的 live＋replay session 全刪（sweep on `.transcriptionDeleted`，鏡射 `reconcileOrphans`）。空 fingerprint 的 session 永不觸碰。 |
| **live／replay 不同時並存**（M9） | 同一錄音**當下**存在 live session → 不提供「產生覆盤」；live 被刪後按鈕回來。狀態判斷，非永久規則。 |
| **覆盤中 tag**（M9） | `MeetingReplayQueue` 的 @Published in-flight 狀態，錄音管理列與覆盤頁 session 列雙邊顯示，完成／失敗即除。執行期狀態，不落盤。 |
| **深答風格**（M9） | Tier 2 輸出密度設定（條列式／簡潔／詳細）。只抽換 `tier2System` 格式指示段——JSON 契約不變，變的是 analysis 字串內部排版。預設條列式：關鍵詞開頭 ≤4 條，3 秒掃視。 |
...
| `MeetingReplayQueue`（M9，spec'd） | 覆盤背景佇列：FIFO 序列（rate-limit 紅線沿用）、同錄音防重複、@Published in-flight 狀態（雙頁「覆盤中」tag 的單一來源）、完成／失敗收尾 | `@MainActor` singleton；`configure(modelContext:)` bootstrap 注入（鏡射 `TranscriptIndexService`）；`MeetingReplayReviewService` 仍是「一場覆盤的執行者」，queue 只管排程＋狀態廣播，view 不再持有 service |
| session reconciler（M9，spec'd） | `.transcriptionDeleted` → sweep 刪 fingerprint 孤兒 session（live＋replay） | 輕量觀察者；nil 批次訊號 → 全量比對（`reconcileOrphans` 的既驗證形狀） |
